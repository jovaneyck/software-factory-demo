...
import { SessionCsvExporter } from './sessions/SessionCsvExporter.js';
...
export function createApp(dataRoot: string = path.join(process.cwd(), 'data')) {
  ...
  const sessionCsvExporter = new SessionCsvExporter(dogRepo, trainingRepo, sessionRepo);
  ...
  app.use('/api', sessionRoutes(dogRepo, sessionRepo, sessionListingService, sessionCsvExporter));
